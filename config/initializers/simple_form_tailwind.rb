...
SimpleForm.setup do |config|
  ...
  config.button_class = 'my-2 bg-blue-500 hover:bg-blue-700 text-white font-bold text-base py-2 px-4 rounded'
  ...
  config.error_notification_class = 'text-white px-6 py-1 border-0 rounded relative mb-4 bg-red-400'
  ...
  config.label_class = 'text-base font-medium text-gray-600 mb-1'
  ...
  config.wrappers :vertical_form, tag: 'div', class: '' do |b|
    ...
    b.use :input,
          class: 'shadow appearance-none border border-gray-300 rounded w-full py-0.5 px-3 bg-white focus:outline-none focus:ring-0 focus:border-blue-500 text-gray-400 transition-colors duration-200 ease-in-out text-base', error_class: 'border-red-500', valid_class: 'border-green-400'
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-1 text-red-500 text-sm' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-1 text-grey-700 text-sm' }
  end
  ...
  config.wrappers :vertical_boolean, tag: 'div', class: 'mb-4 flex items-center', error_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'flex items-center h-5' do |ba|
      ba.use :input,
             class: 'focus:ring-2 focus:ring-indigo-500:focus ring-offset-2 h-4 w-4 text-indigo-600 border-gray-300 rounded mr-2'
    end
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center',
                                        item_label_class: 'my-1 ml-3 block text-base font-medium text-gray-600', tag: 'div', class: 'mb-4' do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: 'legend', class: 'text-base font-medium text-gray-600',
                           error_class: 'text-red-500' do |ba|
      ba.use :label_text
    end
    b.use :input,
          class: 'focus:ring-2 focus:ring-indigo-500 ring-offset-2 h-4 w-4 text-indigo-600 border-gray-300 rounded mr-2 font-medium', error_class: 'text-red-500', valid_class: 'text-green-400'
  ...
  config.wrappers :vertical_file, tag: 'div', class: '' do |b|
    ...
    b.optional :minlength
    b.optional :readonly
    b.use :label, class: 'text-base font-medium text-gray-600 block', error_class: 'text-red-500'
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'my-4', error_class: 'f', valid_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      # ba.use :input, class: 'flex w-auto w-auto text-gray-500 text-base border-gray-300 rounded p-2', error_class: 'text-red-500', valid_class: 'text-green-400'
      ba.use :input,
